...
- Adds retry, error, fallback, or feature-flag paths.
...
For higher-risk telemetry lifecycle logic, tests can assert span creation, status, conditional emission, and exception-path behavior. Apply the instrumentation-only exemption in [false-positive suppression](false-positive-suppression.md).

Prefer descriptive names such as `methodName_condition_expectedResult`. Use fake clocks, deterministic injectable RNGs, latches or coroutine test dispatchers instead of `Thread.sleep`, and `runTest`/`advanceUntilIdle()` for flows. Avoid brittle over-mocking and assertions that test only non-contractual log text.
