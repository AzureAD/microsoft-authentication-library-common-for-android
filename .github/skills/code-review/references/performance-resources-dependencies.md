...
Flag repeated regex compilation/reflection, large temporary allocations in loops, repeated derivation of stable normalized values, accidental quadratic membership/lookups, unbounded parallel command launches, and executor-per-call patterns. Recommend sets/maps for repeated membership checks and move invariant work out of loops.
...
- Coroutines or executors that outlive their owning component.
...
- Major upgrades without relevant release-note/behavior analysis.
- Wildcard versions such as `1.+`.
- Transitive conflicts, especially duplicate telemetry libraries.
- Material DEX/method-count growth.
